-- ============================================================
-- DEFINITIVE POSTGREST SCHEMA CACHE & COLUMN FIX
-- ============================================================
...
BEGIN
    ...
    -- FIX: Ensure duration is TEXT even if user added it as integer manually
    IF (SELECT data_type FROM information_schema.columns WHERE table_name='land_projects' AND column_name='duration') = 'integer' THEN
        ALTER TABLE public.land_projects ALTER COLUMN duration TYPE TEXT USING duration::text;
    ELSIF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='land_projects' AND column_name='duration') THEN
    ...
    -- 5. Mobile App Compatibility Fields (Duplicates for schema sync)
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='land_projects' AND column_name='images') THEN
        ALTER TABLE public.land_projects ADD COLUMN images TEXT[] DEFAULT '{}';
    END IF;
    ...
    -- Update timestamps
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='land_projects' AND column_name='updated_at') THEN
        ALTER TABLE public.land_projects ADD COLUMN updated_at TIMESTAMPTZ DEFAULT now();
    END IF;

END $$;

-- 2. Ensure all rows have defaults
...
UPDATE public.land_projects SET investment_status = 'Active' WHERE investment_status IS NULL;
UPDATE public.land_projects SET is_active = true WHERE is_active IS NULL;

-- 3. FORCED SCHEMA RELOAD
-- Running this command in the SQL editor tells PostgREST to reload the schema immediately.
